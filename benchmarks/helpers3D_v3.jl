...
@views function preprocess(dat_name::String; resx::Int=128, resy::Int=128, do_rotate::Bool=true, do_nondim::Bool=true, fact_nz::Int=2, olen::Int=1)
    ...
    zbed[ismissing.(zbed)] .= nodat_bed
    zbed[zbed.==nodat_bed] .= mean(my_filter(zbed,mask))

    # define surface and avg
    zsurf = zbed .+ zthick
    zavg  = 0.5(zsurf .+ zbed)
    ...
    @parallel set_phases!(ϕ, x3rot, y3rot, z3rot, zsurf2, zbed2, ox, oy, dx, dy)
