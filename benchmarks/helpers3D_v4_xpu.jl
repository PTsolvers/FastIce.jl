...
@views function preprocess(filename::String; resx::Int=128, resy::Int=128, do_nondim::Bool=true, fact_nz::Int=2, ns::Int=4, olen::Int=1)
    ...
    if (me==0) println("- define nx, ny, nz = $nx, $ny, $nz (tx=$tx, ty=$ty, tz=$tz)") end

    # nx_g, ny_g, nz_g = (nx-2)*dims_x+2, (ny-2)*dims_y+2, (nz-2)*dims_z+2
    ...
    # DEBUG: could be done in kernel to have only local arrays...
