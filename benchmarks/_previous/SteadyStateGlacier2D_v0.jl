...
@parallel_indices (ix,iy) function init_ϕ!(ϕ,ybed,ysurf,α,lx,ly,dx,dy)
    xc = -lx/2+dx*ix-dx/2
    yc = -ly/2+dy*iy-dy/2
    xrot = xc*cos(α) - yc*sin(α)
    yrot = xc*sin(α) + yc*cos(α) + ly/2
    if checkbounds(Bool,ϕ,ix,iy)
        if is_inside_fluid(yrot,ysurf[ix])
            ϕ[ix,iy] = fluid
        end
        if is_inside_solid(yrot,ybed[ix])
            ϕ[ix,iy] = solid
        end
    end
    return
...
@views function Stokes2D(Xv, ybed, ysurf)
    ymin   = minimum(ybed)
    ybed   = ybed  .- ymin
    ysurf  = ysurf .- ymin
    ly     = maximum(ysurf)
    ...
    # Yc, Yv    = LinRange(dy/2,ly-dy/2,ny), LinRange(0,ly,ny+1)
    # allocation
    Pt        = @zeros(nx  ,ny  )
    ∇V        = @zeros(nx  ,ny  )
    τxx       = @zeros(nx  ,ny  )
    ...
    Vy_v      = copy(Vy) # visu
    Pt_v      = copy(Pt) # visu
    @parallel init_ϕ!(ϕ,ybed,ysurf,α,lx,ly,dx,dy)
    # iteration loop
...
# load the data
println("Loading the data ... ")
xc, ybed, ysurf = read_data("../data/arolla51.txt"; resol=1024)
println("done.")

Stokes2D(xc, ybed, ysurf)
