...
using Plots, Printf, Statistics, LinearAlgebra, MAT, Random, UnPack
...
@views function Stokes2D(inputs::InputParams)
    @unpack ybed, ysurf, xc, yc, xv, yv, lx_ly, nx, ny, α = inputs
    # physics
    ...
    ρgy       = ρg0*cos(α)
    # numerics
    maxiter   = 10nx         # maximum number of pseudo-transient iterations
    ...
    Vy        = @zeros(nx  ,ny+1)
    ϕ         = fill(air,nx,ny)
    Vx_v      = copy(Vx[2:end,:]) # visu
    Vy_v      = copy(Vy[:,2:end]) # visu
    Pt_v      = copy(Pt) # visu
    ...
    while !((err_V <= ε_V) && (err_∇V <= ε_∇V)) && (iter <= maxiter)
        ...
        if iter % nviz == 0
            Vxa = 0.5*(Vx[1:end-1,:].+Vx[2:end,:])
            Vya = 0.5*(Vy[:,1:end-1].+Vy[:,2:end])
            Vxr = Vxa.*cos(α) .- Vya.*sin(α)
            Vyr = Vxa.*sin(α) .+ Vya.*cos(α)
            Vx_v .= Vxr; Vx_v[Vxr.==0] .= NaN
            Vy_v .= Vyr; Vy_v[Vyr.==0] .= NaN
            ...
            p1 = heatmap(xc,yc,Array(Vx_v)'; c=:batlow, title="Vx", opts...)
            p2 = heatmap(xc,yc,Array(Vy_v)'; c=:batlow, title="Vy", opts...)
...
# load the data
xv, ybed, ysurf = read_data("../data/arolla51.txt"; resol=1024)
