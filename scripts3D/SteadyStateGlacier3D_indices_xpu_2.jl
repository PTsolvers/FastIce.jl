...
@parallel_indices (ix,iy,iz) function compute_P_τ!(∇V, Pt, τxx, τyy, τzz, τxy, τxz, τyz, Vx, Vy, Vz, ϕ, r, μ_veτ, Gdτ, dx, dy, dz)
    ...
    # @not_in_phase_xy ϕ air begin @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ) end
    # @not_in_phase_xz ϕ air begin @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ) end
    # @not_in_phase_yz ϕ air begin @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ) end

    # @in_phases_xy ϕ air fluid fluid fluid begin @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ) end
    # @in_phases_xz ϕ air fluid fluid fluid begin @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ) end
    # @in_phases_yz ϕ air fluid fluid fluid begin @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ) end

    # @in_phases_xy ϕ fluid air fluid fluid begin @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ) end
    # @in_phases_xz ϕ fluid air fluid fluid begin @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ) end
    # @in_phases_yz ϕ fluid air fluid fluid begin @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ) end

    # @in_phases_xy ϕ fluid fluid air fluid begin @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ) end
    # @in_phases_xz ϕ fluid fluid air fluid begin @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ) end
    # @in_phases_yz ϕ fluid fluid air fluid begin @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ) end

    # @in_phases_xy ϕ fluid fluid fluid air begin @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ) end
    # @in_phases_xz ϕ fluid fluid fluid air begin @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ) end
    # @in_phases_yz ϕ fluid fluid fluid air begin @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ) end
    ...
    if ix <= size(ϕ,1)-1 && iy <= size(ϕ,2)-1 && iz <= size(ϕ,3)-2
        nair = 0; nfluid = 0
        if ϕ[ix  ,iy  ,iz+1] == air nair += 1 elseif ϕ[ix  ,iy  ,iz+1] == fluid nfluid += 1 end
        if ϕ[ix+1,iy  ,iz+1] == air nair += 1 elseif ϕ[ix+1,iy  ,iz+1] == fluid nfluid += 1 end
        if ϕ[ix  ,iy+1,iz+1] == air nair += 1 elseif ϕ[ix  ,iy+1,iz+1] == fluid nfluid += 1 end
        if ϕ[ix+1,iy+1,iz+1] == air nair += 1 elseif ϕ[ix+1,iy+1,iz+1] == fluid nfluid += 1 end
        if nair < 2 && nfluid > 0
            @all(τxy) = μ_veτ*(@d_yi(Vx)/dy + @d_xi(Vy)/dx + @all(τxy)/Gdτ)
        end
    end

    if ix <= size(ϕ,1)-1 && iy <= size(ϕ,2)-2 && iz <= size(ϕ,3)-1
        nair = 0; nfluid = 0
        if ϕ[ix  ,iy+1,iz  ] == air nair += 1 elseif ϕ[ix  ,iy+1,iz  ] == fluid nfluid += 1 end 
        if ϕ[ix+1,iy+1,iz  ] == air nair += 1 elseif ϕ[ix+1,iy+1,iz  ] == fluid nfluid += 1 end 
        if ϕ[ix  ,iy+1,iz+1] == air nair += 1 elseif ϕ[ix  ,iy+1,iz+1] == fluid nfluid += 1 end 
        if ϕ[ix+1,iy+1,iz+1] == air nair += 1 elseif ϕ[ix+1,iy+1,iz+1] == fluid nfluid += 1 end 
        if nair < 2 && nfluid > 0
            @all(τxz) = μ_veτ*(@d_zi(Vx)/dz + @d_xi(Vz)/dx + @all(τxz)/Gdτ)
        end
    end

    if ix <= size(ϕ,1)-2 && iy <= size(ϕ,2)-1 && iz <= size(ϕ,3)-1
        nair = 0; nfluid = 0
        if ϕ[ix+1,iy  ,iz  ] == air nair += 1 elseif ϕ[ix+1,iy  ,iz  ] == fluid nfluid += 1 end
        if ϕ[ix+1,iy  ,iz+1] == air nair += 1 elseif ϕ[ix+1,iy  ,iz+1] == fluid nfluid += 1 end
        if ϕ[ix+1,iy+1,iz  ] == air nair += 1 elseif ϕ[ix+1,iy+1,iz  ] == fluid nfluid += 1 end
        if ϕ[ix+1,iy+1,iz+1] == air nair += 1 elseif ϕ[ix+1,iy+1,iz+1] == fluid nfluid += 1 end
        if nair < 2 && nfluid > 0
            @all(τyz) = μ_veτ*(@d_zi(Vy)/dz + @d_yi(Vz)/dy + @all(τyz)/Gdτ)
        end
    end
    return
end
...
@parallel_indices (ix,iy,iz) function preprocess_visu!(Vn, τII, ϕ, Vx, Vy, Vz, τxx, τyy, τzz, τxy, τxz, τyz)
    # all arrays of size (nx-2,ny-2,nz-2)
    nfluid_x = 0; nfluid_y = 0; nfluid_z = 0
    vxc = 0.0; vyc = 0.0; vzc = 0.0
    if ix <= size(Vn,1) && iy <= size(Vn,2) && iz <= size(Vn,3)
        nfluid_x = (ϕ[ix+1,iy+1,iz+1] == fluid) + (ϕ[ix+2,iy+1,iz+1] == fluid)
        vxc      = Vx[ix+1,iy+1,iz+1]*(ϕ[ix+1,iy+1,iz+1] == fluid) + Vx[ix+2,iy+1,iz+1]*(ϕ[ix+2,iy+1,iz+1] == fluid)

        nfluid_y = (ϕ[ix+1,iy+1,iz+1] == fluid) + (ϕ[ix+1,iy+2,iz+1] == fluid)
        vyc = Vy[ix+1,iy+1,iz+1]*(ϕ[ix+1,iy+1,iz+1] == fluid) + Vy[ix+1,iy+2,iz+1]*((ϕ[ix+1,iy+2,iz+1] == fluid))

        nfluid_z = (ϕ[ix+1,iy+1,iz+1] == fluid) + (ϕ[ix+1,iy+1,iz+2] == fluid)
        vzc      = Vz[ix+1,iy+1,iz+1]*(ϕ[ix+1,iy+1,iz+1] == fluid) + Vz[ix+1,iy+1,iz+2]*(ϕ[ix+1,iy+1,iz+2] == fluid)

        if (nfluid_x > 0)  vxc /= nfluid_x  end
        if (nfluid_y > 0)  vyc /= nfluid_y  end
        if (nfluid_z > 0)  vzc /= nfluid_z  end

        Vn[ix,iy,iz] = sqrt(vxc*vxc + vyc*vyc + vzc*vzc)
    end
...
@views function Stokes3D(dem)
    ...
    if do_save
        ...
        @parallel preprocess_visu!(Vn, τII, ϕ, Vx, Vy, Vz, τxx, τyy, τzz, τxy, τxz, τyz)
