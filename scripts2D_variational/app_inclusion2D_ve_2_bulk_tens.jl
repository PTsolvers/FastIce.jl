...
@views function runsim(::Type{DAT}; nx=127) where {DAT}
    ...
    Ps = P[iP:end]
    Pt = P[1:iP]
    τIIs = C0 * cosϕs                    .+  Ps         .* sinϕs
    τIIt = C0 * cosϕs .+ P_y .* sind(ϕs) .+ (Pt .- P_y) .* tanϕt
    ...
    for it in 1:nt
        ...
        while any(errs .>= ϵtol) && (iter += 1) <= maxiter
            ...
            update_τ!(Pr, Pr_c, ε_ve, τ, ηs, η_ve, G, K, dt, τII, Ft, Fs, λt, λs, Γ, C, cosϕs, P_y, sinϕs, tanϕt, sinψs, tanψt, η_reg, χλ, θ_dτ, wt)
            compute_Fchk_xII_η!(τII, Fchk, εII, ηs, Pr_c, τ, ε, λt, λs, Γ, C, cosϕs, sinϕs, tanϕt, η_reg, wt, χ, mpow, ηmax)
