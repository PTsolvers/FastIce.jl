@tiny function _kernel_increment_τ!(Pr, ε, δτ, τ, τ_o, V, ηs, G, dt, wt, r, θ_dτ, dx, dy)
    ix, iy = @indices
...
@tiny function _kernel_update_τ!(Pr, ε, δτ, τ, τ_o, ηs, G, dt, τII, F, λ, τ_y, sinϕ, η_reg, χλ, θ_dτ, wt)
    ix, iy = @indices
    @inline isin(A) = checkbounds(Bool, A, ix, iy)
    @inbounds if isin(Pr)
        # detect and eliminate null spaces
