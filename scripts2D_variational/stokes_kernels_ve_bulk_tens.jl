...
@tiny function _kernel_update_τ!(Pr, Pr_c, ε, δτ, τ, τ_o, ηs, G, K, dt, τII, Ft, Fs, λt, λs, τ_y, P_y, sinϕs, tanϕt, sinψs, tanψt, η_reg, χλ, θ_dτ, wt)
    ix, iy = @indices
    @inline isin(A) = checkbounds(Bool, A, ix, iy)
    @inbounds if isin(Pr)
        # detect and eliminate null spaces
        ...
        if !isnull && (wt.not_air.c[ix, iy] > 0.0)
            ...
            Γ1 = (Ft[ix, iy] > 0.0) ? 1 : 0
            Γ2 = (Fs[ix, iy] > 0.0) ? 2 : 0
            Γ3 = (Fs[ix, iy] > 0.0 && Ft[ix, iy] > 0.0) ? 3 : 0
            # corrections
            λt[ix, iy] = (Γ1 == 1) ? ((1.0 - χλ) * λt[ix, iy] + χλ * (max(Ft[ix, iy], 0.0) / (ηs[ix, iy] * dτ_r + η_reg + K * dt * tanϕt * tanψt))) : 0.0
            λs[ix, iy] = (Γ2 == 2) ? ((1.0 - χλ) * λs[ix, iy] + χλ * (max(Fs[ix, iy], 0.0) / (ηs[ix, iy] * dτ_r + η_reg + K * dt * sinϕs * sinψs))) : 0.0
            ...
            τ.xx[ix, iy]  += (-(τ.xx[ix, iy]  - τ_o.xx[ix, iy])  * ηs[ix, iy] / (G * dt) - τ.xx[ix, iy]  + 2.0 * ηs[ix, iy] * (ε.xx[ix, iy] - ∇V / 3.0 -       (λt[ix, iy] + λs[ix, iy]) * dQdτxx)) * dτ_r
            τ.yy[ix, iy]  += (-(τ.yy[ix, iy]  - τ_o.yy[ix, iy])  * ηs[ix, iy] / (G * dt) - τ.yy[ix, iy]  + 2.0 * ηs[ix, iy] * (ε.yy[ix, iy] - ∇V / 3.0 -       (λt[ix, iy] + λs[ix, iy]) * dQdτyy)) * dτ_r
            τ.xyc[ix, iy] += (-(τ.xyc[ix, iy] - τ_o.xyc[ix, iy]) * ηs[ix, iy] / (G * dt) - τ.xyc[ix, iy] + 2.0 * ηs[ix, iy] * (ε.xyc[ix, iy]           - 0.5 * (λt[ix, iy] + λs[ix, iy]) * dQdτxy)) * dτ_r
            # missing: if (Γ3 == 3) τii = τ_y
            Pr_c[ix, iy] = Pr[ix, iy] + K * dt * (λt[ix, iy] * tanψt + λs[ix, iy] * sinψs)
            Pr_c[ix, iy] = (Γ3 == 3) ? P_y : Pr_c[ix, iy]
        else
            τ.xx[ix, iy]  = 0.0
            τ.yy[ix, iy]  = 0.0
            τ.xyc[ix, iy] = 0.0
