...
@views function runsim(::Type{DAT}; nx=127) where {DAT}
    ...
    while ε̇bg < ε̇bgTot
        ...
        while any(errs .>= ϵtol) && (iter += 1) <= maxiter
            increment_τ!(Pr, ε, δτ, τ, V, ηs, wt, r, θ_dτ, dτ_r, dx, dy)
            compute_xyc!(ε, δτ, τ, ηs, wt, dτ_r)
            compute_trial_τII!(τII, δτ, τ)
            update_τ!(Pr, ε, δτ, τ, ηs, τII, F, λ, τ_y, sinϕ, η_reg, χλ, wt, dτ_r)
            ...
            if iter % ncheck == 0
                ...
                # plt.fields[4][3] = to_host(Ψ.not_air)
                display(fig)
            end
        end
    end
