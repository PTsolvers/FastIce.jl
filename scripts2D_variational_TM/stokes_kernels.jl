@tiny function _kernel_update_σ!(Pr, τ, V, ηs, wt, r, θ_dτ, dτ_r, dx, dy)
    ...
    if !isnull && (na.c[ix, iy] > 0.0)
        εxx = (V.x[ix+1, iy] * ns.x[ix+1, iy] - V.x[ix, iy] * ns.x[ix, iy]) / dx
        εyy = (V.y[ix, iy+1] * ns.y[ix, iy+1] - V.y[ix, iy] * ns.y[ix, iy]) / dy
        ∇V = εxx + εyy
        Pr[ix, iy] -= ∇V * ηs[ix, iy] * r / θ_dτ
        τ.xx[ix, iy] += (-τ.xx[ix, iy] + 2.0 * ηs[ix, iy] * (εxx - ∇V / 3.0)) * dτ_r
        τ.yy[ix, iy] += (-τ.yy[ix, iy] + 2.0 * ηs[ix, iy] * (εyy - ∇V / 3.0)) * dτ_r
    else
        Pr[ix, iy] = 0.0
        τ.xx[ix, iy] = 0.0
        τ.yy[ix, iy] = 0.0
    ...
    @inbounds if isin(τ.xy)
        ...
        if !isnull && (na.xy[ix, iy] > 0.0)
            ...
            τ.xy[ix, iy] += (-τ.xy[ix, iy] + 2.0 * ηs_av * εxy) * dτ_r
        else
            τ.xy[ix, iy] = 0.0
        end
    end
