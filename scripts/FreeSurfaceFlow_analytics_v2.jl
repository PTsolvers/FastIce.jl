...
@views function Stokes2D()
    ...
    nx, ny    = nx-1, ny-1
    maxiter   = 20ny         # maximum number of pseudo-transient iterations
    ...
    while !((err_V <= ε_V) && (err_∇V <= ε_∇V)) && (iter <= maxiter)
        ...
        if iter % nviz == 0
            ...
            Vmc_exact  = ρg0*sin(β)./μs0.*Yc_tr.*(Yc_tr./2 .- h_exact)
            p5 = plot(Yv[2:end-1],Vmc_slice; label="numerical",linewidth=2,xlims=(0.0,gl-el),ylims=(0.0,0.04))
            scatter!(Yc_exact, Vmc_exact; label="exact",marker=:d,xlims=(0.0,gl-el),ylims=(0.0,0.04))
