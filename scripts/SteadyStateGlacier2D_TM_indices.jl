...
include(joinpath(@__DIR__, "flagging_macros2D.jl"))
...
macro fm_xi(A) esc(:( ($A[ix,iy+1] == fluid) && ($A[ix+1,iy+1] == fluid) )) end
macro fm_yi(A) esc(:( ($A[ix+1,iy] == fluid) && ($A[ix+1,iy+1] == fluid) )) end
...
@parallel_indices (ix,iy) function compute_V_T!(Vx, Vy, T, μs, Pt, τxx, τyy, τxy, EII, T_o, qTx, qTy, ϕ, μs0, ρgx, ρgy, Ta, Q_R, T0, dt, vpdτ_mech, max_lxy, Re_mech, dτ_ρ_heat, dx, dy)
    ...
    @not_in_phases_yi ϕ solid solid begin
        @inn(Vy) = @inn(Vy) + @dτ_ρ_mech_ay()*(@d_yi(τyy)/dy + @d_xa(τxy)/dx - @d_yi(Pt)/dy - @fm_yi(ϕ)*ρgy)
    end
    # thermo
    @within ϕ begin
...
@parallel_indices (ix,iy) function compute_Res!(Rx, Ry, RT, Pt, τxx, τyy, τxy, T, T_o, qTx, qTy, EII, μs, ϕ, ρgx, ρgy, dt, dx, dy)
    ...
    @not_in_phases_yi ϕ solid solid begin
        @all(Ry) = @d_yi(τyy)/dy + @d_xa(τxy)/dx - @d_yi(Pt)/dy - @fm_yi(ϕ)*ρgy
    end
    # thermo
    @within ϕ begin
...
macro av_xii(A) esc(:( 0.5*($A[ix+1,iy+1] + $A[ix+2,iy+1]) )) end
