...
function main(backend = CPU(), T::DataType = Float64, dims = (0, 0, 0))
    ...
    # A_ini .= [exp(-(x_g(ix, dx) - l/2)^2 - (y_g(iy, dy) - l/2)^2 - (z_g(iz, dz) - l/2)^2) for ix=1:size(A,1), iy=1:size(A,2), iz=1:size(A,3)]
    ...
    #             diffusion_kernel!(backend, 256)(A_new, A, h, dx, dy, dz, first(ranges[I]); ndrange=size(ranges[I]))
    #             KernelAbstractions.synchronize(backend)
