...
function volfrac(rect::Rect2{T}, ϕ::Vec4{T}) where {T}
    or, ws = Grids.origin(rect), widths(rect)
...
function volfrac(rect::Rect3, ϕ::Vec{8})
    or, ws = GeometryBasics.origin(rect), widths(rect)
    v000, v001, v100, v101 = or, or + Vec(ws[1], 0.0, 0.0), or + Vec(0.0, ws[2], 0.0), or + Vec(ws[1], ws[2], 0.0)
    v010, v011, v110, v111 = or + Vec(0.0, 0.0, ws[3]), or + Vec(ws[1], 0.0, ws[3]), or + Vec(0.0, ws[2], ws[3]),
                             or + Vec(ws[1], ws[2], ws[3])
    ϕ = perturb.(ϕ)
