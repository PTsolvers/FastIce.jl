...
@kernel inbounds = true function update_V!(V, Pr, τ, η, η_next, rheology, ρg, Δτ, g::StructuredGrid{2}, O=Offset())
    ...
    I += O
    # velocity
    V.x[I] += (-∂x(Pr, g, I) + ∂x(τ.xx, g, I) + ∂y(τ.xy, g, I) - ρg.x[I]) / lerp(η, location(V.x), I) * Δτ.V.x
    V.y[I] += (-∂y(Pr, g, I) + ∂y(τ.yy, g, I) + ∂x(τ.xy, g, I) - ρg.y[I]) / lerp(η, location(V.x), I) * Δτ.V.y
    # rheology
...
@kernel inbounds = true function compute_τ!(τ, V, η, g::StructuredGrid{2}, O=Offset())
    I = @index(Global, Cartesian)
    I += O
    ε̇xx = ∂x(V.x, g, I)
    ε̇yy = ∂y(V.y, g, I)
...
@kernel inbounds = true function compute_residuals!(r_V, r_Pr, Pr, τ, V, ρg, g::StructuredGrid{2}, O=Offset())
    I = @index(Global, Cartesian)
    I += O
    # pressure
