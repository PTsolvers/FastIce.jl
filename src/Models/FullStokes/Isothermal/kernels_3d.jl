...
@kernel inbounds = true function update_V!(V, Pr, τ, η, η_next, rheology, ρg, Δτ, g::StructuredGrid{3}, O=Offset())
    I = @index(Global, Cartesian)
    I += O
    # velocity
    V.x[I] += (-∂x(Pr, g, I) + ∂x(τ.xx, g, I) + ∂y(τ.xy, g, I) + ∂z(τ.xz, g, I) - ρg.x[I]) / lerp(η, location(V.x), g, I) * Δτ.V.x
    V.y[I] += (-∂y(Pr, g, I) + ∂y(τ.yy, g, I) + ∂x(τ.xy, g, I) + ∂z(τ.yz, g, I) - ρg.y[I]) / lerp(η, location(V.y), g, I) * Δτ.V.y
    V.z[I] += (-∂z(Pr, g, I) + ∂z(τ.zz, g, I) + ∂x(τ.xz, g, I) + ∂y(τ.yz, g, I) - ρg.z[I]) / lerp(η, location(V.z), g, I) * Δτ.V.z
    # rheology
...
@kernel inbounds = true function compute_τ!(τ, V, η, g::StructuredGrid{3}, O=Offset())
    I = @index(Global, Cartesian)
    I += O
    # strain rates
    ε̇xx = ∂x(V.x, g, I)
...
@kernel inbounds = true function compute_residuals!(r_V, r_Pr, Pr, τ, V, ρg, g::StructuredGrid{3}, O=Offset())
    I = @index(Global, Cartesian)
    I += O
    # pressure
