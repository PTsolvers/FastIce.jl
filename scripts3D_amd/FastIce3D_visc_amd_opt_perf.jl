...
@views function main(;do_visu=false,do_save=false,outdir="out_$(randstring(3))")
    # physics
    ...
    maxiter    = 100#100min(nx,ny,nz)
    ncheck     = 1000#ceil(Int,5min(nx,ny,nz))
    r          = 0.6
    re_mech    = 5.2π
    nt         = 1
    println("Process selecting device $(AMDGPU.default_device_id())")
    println("Problem size: nx=$nx, ny=$ny, nz=$nz")
    ...
    for it = 1:nt
        ...
        while any(errs .>= ϵtol) && iter <= maxiter
            if (iter>maxiter-10) t_tic = time_ns() end
    ...
    if do_save
        ...
        Vmag .= sqrt.(ameanx(Vx).^2 .+ ameany(Vy).^2 .+ ameanz(Vz).^2)
        save_array(joinpath(outdir,"out_Pr"  ),convert.(Float32,Array(Pr)  ))
        save_array(joinpath(outdir,"out_Vmag"),convert.(Float32,Array(Vmag)))
        save_array(joinpath(outdir,"out_T"   ),convert.(Float32,Array(T)   ))
